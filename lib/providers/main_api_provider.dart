...
class MainApi with ChangeNotifier {
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        if (responseArray['results'] == null) {
          ...
          for (var round in responseArray['results']['overall']['tables']) {

            List<RowsModel> rows = [];

            for (var singleResult in round['rows']) {
              RowsModel row = RowsModel(
            ...
            PositionModel position =
                PositionModel(groupName: round['groupname'] ?? '', rows: rows);
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        for (var singleResult in responseArray['games_pre']) {
          ...
          if (oddResponse.statusCode == 200) {
            ...
            try {
              oddHome = (double.parse(array['results']['Bet365']['odds']['end']['18_2']
              ['home_od'])).toStringAsFixed(2) ??
                  'n/a';
              // oddX = array['results']['Bet365']['odds']['end']['18_2']
              // ['handicap'] ??
              //     'n/a';
              oddAway = (double.parse(array['results']['Bet365']['odds']['end']['18_2']
              ['away_od'])).toStringAsFixed(2) ??
                  'n/a';
            } catch (error) {
              oddHome =
  ...
  Future<void> getHeadTwoHeadData(
      context, String leagueId, String gameId) async {
    try {
      ...
      if (previewResponse.statusCode == 201 ||
        ...
        Navigator.push(context,
            MaterialPageRoute(builder: (context) => Preview(leagueId, gameId)));
      } else {
        // If the server did not return a 201 CREATED response,
        // then throw an exception.
        _apiState = ApiState.loaded;
  ...
  Future<void> getOvers([int leagueId = 0]) async {
    ...
    try {
      ...
      var url =
          '$apiEndPoint?login=$apiLogin&token=$apiToken&task=enddata&sport=basketball&league=$leagueId&p=1';
      // print(url);
      overs = [];
      notifyListeners();
      final response = await http.get(Uri.parse(url), headers: <String, String>{
  ...
  Future<void> getStatisticsData(
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        statistics.p3Points =
            responseArray['results'][0]['stats']['2points'] ?? ['n/a', 'n/a'];
        statistics.biggestLead = responseArray['results'][0]['stats']['biggest_lead'] ?? ['n/a', 'n/a'];
        statistics.fouls =
        ...
        statistics.freeThrowsRate = responseArray['results'][0]['stats']
                ['free_throws_rate'] ??
            ['n/a', 'n/a'];
        statistics.leadChanges = responseArray['results'][0]['stats']['lead_changes'] ?? ['n/a', 'n/a'];
        statistics.maxPointsInARow = responseArray['results'][0]['stats']['maxpoints_inarow'] ?? ['n/a', 'n/a'];
        statistics.possession = responseArray['results'][0]['stats']['possession'] ?? ['n/a', 'n/a'];
        statistics.timeSpentInLead = responseArray['results'][0]['stats']['timespent_inlead'] ?? ['n/a', 'n/a'];
        statistics.timeOuts =
        ...
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => Statistics(leagueId, gameId)));
      } else {
        // If the server did not return a 201 CREATED response,
        // then throw an exception.
        _apiState = ApiState.loaded;
      ...
      if (interestLists.length > 1) {
        ...
        for (var interest in interestLists.reversed) {
          ...
          if(interest.toString().trim() != ''){
            var gameId = interest;
            String url = '$apiEndPoint?login=$apiLogin&token=$apiToken&task=eventdata&game_id=${gameId.toString()}';
            final response = await http.get(
                Uri.parse(url),
                headers: <String, String>{
                  'Content-Type': 'application/json; charset=UTF-8',
  ...
  Future<void> addRemoveInterests(
      context, int index, String gameId, bool isFavorite) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    var interestsList = prefs.getStringList('interests');
    if (isFavorite) {
      //already exists, remove it
  ...
  Future<void> addRemoveInterestsOvers(
      context, int index, String gameId, bool isFavorite) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    var interestsList = prefs.getStringList('interests');
    if (isFavorite) {
